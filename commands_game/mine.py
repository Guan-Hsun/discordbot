...
class MineControlView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        if self.message:
            ...
            embed.set_footer(text="⚠️ 操作介面已過期，請重新輸入指令喚醒。")
            try:
    ...
    async def sell_ores(self, interaction, target_ore):
        ...
        if target_ore == "all":
            current_ores = sum(self.p_data["ores"].values())
            if current_ores == 0:
                self.status_msg = "⚠️ 你的倉庫空空如也，沒有東西可以賣！"
        ...
        self.status_msg = f"✅ **交易成功！** 賣出礦石實拿 **{final_value}** 傑尼幣<:emoji_1:1127882888508088393>！"
        await self.update_message(interaction)
    ...
    async def do_upgrade(self, interaction, target):
        ...
        if max_lv:
            self.status_msg = "⚠️ 該項目已經達到最高等級！"
            return await self.update_message(interaction)

        if total_m < req:
            self.status_msg = f"⚠️ **經驗不足！** 需累計挖滿 {req} 顆礦石才能解鎖。"
            return await self.update_message(interaction)

        if wallet < cost:
            self.status_msg = f"⚠️ **餘額不足！** 升級需要 {cost} 傑尼幣<:emoji_1:1127882888508088393>。"
            return await self.update_message(interaction)
        ...
        self.status_msg = f"✅ **升級成功！** 已扣除 {cost} 傑尼幣<:emoji_1:1127882888508088393>。"
        await self.update_message(interaction)
